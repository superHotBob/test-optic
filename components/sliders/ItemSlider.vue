  ...
  <div class="item-slider" v-if="returnItems">
    <div class="item-slider__categories" v-swiper:mySwiperC="categoriesOption">
      <div v-if="items == 'recommended'">
        <p class="simple active">
          <slot>Рекомендуем вам</slot>
        </p>
      </div>
      <div class="swiper-wrapper" v-else>
        ...
        >Бестселлеры</p>
        <p
          v-if="itemCategory === 'mostWanted' && returnItems"
          class="swiper-slide"
      ...
      <div class="item-slider__controls">
        <button class="item-slider__prev" @click="prev()">Назад</button>
        <button class="item-slider__next" @click="next()">Вперёд</button>
      </div>
    </div>
    <div class="item-slider__items" v-swiper:mySwiper="swiperOption">
      <div class="swiper-wrapper">
        <item v-for="(item, index) in returnItems" :key="index" :item="item" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    returnItems() {
      ...
      if (this.items == "recommended") return this.bestsellers; // нужно подставить правильный геттер

      if (this.itemCategory == "bestsellers") return this.bestsellers;
      if (this.itemCategory == "newItems") return this.newItems;
      if (this.itemCategory == "mostWanted") return this.bestsellers;
